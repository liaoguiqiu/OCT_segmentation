...
class Generator_Contour_layers(object):
    def __init__(self ):
        ...
        self.save_img_dir = "../../"     + "saved_generated_layers/"
        self.save_contour_dir = "../../"     + "saved_stastics_layer_generated/"
    ...
    def generate(self):
        ...
        for n in range(500):
            #for num in range(file_len):
                ...
                W_new = 1024
                #generate the signal 
                contourx , contoury   = Basic_Operator.random_shaped_layer(H,W) 
                fake= line_syn.fake_image(H,W,contourx,contoury)
                ...
                fake = np.clip(fake, 1, 254)
                cv2.imshow('w1',fake.astype(np.uint8))

                #self.display_contour(new_image,new_contourx,new_contoury,'warped')  
                display  = Basic_Operator.gray2rgb(fake)
                for  drwer in range(4):
                    display  = Basic_Operator.draw_coordinates_color(display,contourx[drwer],contoury[drwer],1)
                ...
                print(str(img_id))
                self.append_new_name_contour(img_id,contourx,contoury,self.save_contour_dir)
                cv2.imwrite(self.save_img_dir  + str(img_id) +".jpg",fake )
                img_id +=1
...
if __name__ == '__main__':
    generator  = Generator_Contour_layers()
